...
import com.createver.server.domain.music.dto.response.AlbumResponse;
import com.createver.server.domain.music.dto.response.MusicResponse;
...
public class MemberController {
    ...
    @GetMapping("/my-music")
    public Response<Page<MusicResponse>> getMyMusic(Authentication authentication, Pageable pageable) {
        Page<MusicDto> musicDtos = memberService.getMyMusic(authentication.getName(), pageable);
        return Response.success(musicDtos.map(MusicResponse::from));
    }

    @GetMapping("/my-album")
    public Response<Page<AlbumResponse>> getMyAlbum(Authentication authentication, Pageable pageable) {
        Page<AlbumDto> albumDtos = memberService.getMyAlbum(authentication.getName(), pageable);
        return Response.success(albumDtos.map(AlbumResponse::from));
    }
